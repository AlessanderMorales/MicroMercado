...
_factory = factory;
 ...
 #region IT-40: No se puede eliminar categoría con productos asociados
     ...
     [Fact(DisplayName = "IT-40: No se puede eliminar categoría con productos asociados")]
        public async Task DeleteCategory_WithAssociatedProducts_ShouldFail()
        {
  ...
  var product = await productService.CreateProductAsync(productDto);
      ...
      // Act & Assert - Intentar eliminar la categoría debe lanzar excepción por integridad referencial
            await Assert.ThrowsAsync<DbUpdateException>(async () =>
 {
          await categoryService.DeleteCategoryAsync(category.Id);
         });

      // Verificar que la categoría sigue existiendo
            var categoryStillExists = await context.Categories
        .AnyAsync(c => c.Id == category.Id);
            Assert.True(categoryStillExists);
        }
   ...
   [Fact(DisplayName = "IT-41: No se puede eliminar producto que está en una venta")]
        public async Task DeleteProduct_WithAssociatedSales_ShouldFail()
        {
using var scope = _factory.Services.CreateScope();
 ...
 Change = 0m,
           Items = new List<SaleDTO.SaleItemDTO>
   {
    ...
    Assert.True(saleResult.Success);
      ...
      // Act & Assert - Intentar eliminar el producto debe lanzar excepción por integridad referencial
          await Assert.ThrowsAsync<DbUpdateException>(async () =>
 {
            ...
            var productStillExists = await context.Products.AnyAsync(p => p.Id == 1);
   Assert.True(productStillExists);
        }
        ...
        [Fact(DisplayName = "IT-42: No se puede eliminar cliente que tiene ventas asociadas")]
        public async Task DeleteClient_WithAssociatedSales_ShouldFail()
      {
      using var scope = _factory.Services.CreateScope();
       var saleService = scope.ServiceProvider.GetRequiredService<ISaleService>();
 ...
 // Act & Assert - Intentar eliminar el cliente debe lanzar excepción por integridad referencial
          await Assert.ThrowsAsync<DbUpdateException>(async () =>
     {
   ...
   var clientStillExists = await context.Clients.AnyAsync(c => c.Id == 1);
            Assert.True(clientStillExists);
        }
        ...
        [Fact(DisplayName = "IT-40b: Eliminar categoría sin productos debe ser exitoso")]
